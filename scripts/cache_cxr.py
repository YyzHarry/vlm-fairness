...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, choices=['mimic', 'vindr'])
    ...
    if args.dataset == 'mimic':
        ...
        assert (mimic_dir/'foundation_fair_meta'/'metadata.csv').is_file(), \
            "Please run download.py to generate the metadata for `mimic_cxr` first!"
        df = pd.read_csv(mimic_dir/'foundation_fair_meta'/'metadata.csv')
    elif args.dataset == 'vindr':
        vin_dir = Path(os.path.join(args.data_path, "vindr-cxr"))
        assert (vin_dir/'foundation_fair_meta'/'metadata.csv').is_file(), \
            "Please run download.py to generate the metadata for `vindr` first!"
        df = pd.read_csv(vin_dir/'foundation_fair_meta'/'metadata.csv')
    else:
